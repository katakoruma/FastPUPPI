...
L1PFDecodedCaloTableProducer::produce(edm::StreamID id, edm::Event& iEvent, const edm::EventSetup& iSetup) const
{
    ...
    for (unsigned int i = 0; i < ncands; ++i) {
        ...
        if(hgcalcl) {
            ...
            vals_firstlayer[i] = hgcalcl->firstLayer();
            vals_maxlayer[i] = hgcalcl->maxLayer();
            vals_sigmarr[i] = hgcalcl->sigmaRRTot();
            vals_hw_sigmarr[i] = hgcalcl->sigmaRRTot();
            vals_sigmarrmax[i] = hgcalcl->sigmaRRMax();
            vals_hw_sigmarrmax[i] = hgcalcl->sigmaRRMax();
            vals_sigmarrmean[i] = hgcalcl->sigmaRRMean();
            vals_hw_sigmarrmean[i] = hgcalcl->sigmaRRMean();
            vals_zbarycenter[i] = hgcalcl->zBarycenter();
            vals_ebm0[i] = hgcalcl->ebm0();
            vals_ebm1[i] = hgcalcl->ebm1();
            vals_hbm[i] = hgcalcl->hbm();
            vals_first1layers[i] = hgcalcl->first1layers();
            vals_first3layers[i] = hgcalcl->first3layers();
            vals_first5layers[i] = hgcalcl->first5layers();
            vals_firstHcal1layers[i] = hgcalcl->firstHcal1layers();
            vals_firstHcal3layers[i] = hgcalcl->firstHcal3layers();
            vals_firstHcal5layers[i] = hgcalcl->firstHcal5layers();
            vals_last1layers[i] = hgcalcl->last1layers();
            vals_last3layers[i] = hgcalcl->last3layers();
            vals_last5layers[i] = hgcalcl->last5layers();
        }
    }
